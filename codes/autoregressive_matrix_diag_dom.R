...
# list of A_matrices
autoregressive_matrix_own_other_diag_dom <- function(d,p,L,max_eigen_abs){
  A_matrix_list <- lapply(1:p, function(x) matrix(rnorm(n = d^2,mean=10,sd=10),nrow=d,ncol=d))
  
  # Replace the diagonal of each matrix row by its maximum value+1:
  for(i in 1:length(A_matrix_list)){
    max_val <- unlist(lapply(1:nrow(A_matrix_list[[i]]),function(x) max(abs(A_matrix_list[[i]][x,]))))
    diag(A_matrix_list[[i]]) <- max_val+1
  }
...
# d <- 5
# p <- 3
# L <- matrix(c(c(3,2,2,2,2),c(1,2,1,1,1),c(3,3,3,3,3),c(3,3,3,3,3),c(1,1,1,1,2)),byrow = T,nrow=d,ncol = d)
# max_eigen_abs <- 0.95
# mat <- autoregressive_matrix_own_other_diag_dom(d = d,p = p,L = L,max_eigen_abs = max_eigen_abs)
# 
